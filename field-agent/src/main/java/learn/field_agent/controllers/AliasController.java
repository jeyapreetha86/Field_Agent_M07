...
public class AliasController {
    ...
    @GetMapping("/agent/{aliasName}")
    public ResponseEntity<List<AgentAlias>> findByAlias(@PathVariable String aliasName) {
        List<AgentAlias> agents = service.findByAlias(aliasName);
        if (agents == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
